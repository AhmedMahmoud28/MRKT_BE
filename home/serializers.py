...
class AddtoWishlistserializer(serializers.ModelSerializer):
    ...
    def save(self, **kwargs):
        with transaction.atomic(): 
            ...
            self.instance = models.Wishlist.objects.select_related('product').filter(user_id=user_id, 
                                                                                     product__id=product_added.id).first()
            if self.instance is None:
                return models.Wishlist.objects.select_related('product').create(user_id=user_id, 
...
class AddReviewSerializer(serializers.ModelSerializer):
    ...
    def save(self, **kwargs):
        with transaction.atomic(): 
            ...
            self.instance = models.Review.objects.select_related('product').filter(user_id=user_id, 
                                                                                   product__id=product_added.id).first()
            if self.instance is None:
                return models.Review.objects.select_related('product').create(user_id=user_id, 
                                                                                product=product_added, 
                                                                                rate=rate_added,
